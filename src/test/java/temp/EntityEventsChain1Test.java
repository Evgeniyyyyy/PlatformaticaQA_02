...
public class EntityEventsChain1Test extends BaseTest {
    ...
    private <T> T returnNumericValue(String value) {
        if (value != null && isNumeric(value)) {
            ...
            if (stringParsed >= 0) {
                if (stringParsed <= 4194303) return (T) Integer.valueOf(value);
                else return (T) Long.valueOf(stringParsed);
            } else {
                return null;
            }
        }
    ...
    public void createChain1DraftRecord() {
        ...
        for (int i = 0; i < 9; i++) {
            int cellI = Integer.parseInt(cells.get(i));
            int cellINext = Integer.parseInt(cells.get(i + 1));
            Assert.assertEquals(
                    cellI * 2,
                    cellINext,
